...
const Assets = () => {
  ...
  const { data, isFetching, isError, isSuccess } = useAssetsQuery(params);
  const { enqueueSnackbar } = useSnackbar();

  const [archiveAsset, { isLoading: loadingArchive }] =
    useArchiveAssetsMutation();

  const [importAsset, { isLoading: loadingImport }] = useImportAssetsMutation();
  ...
  const importHeader = [
    { name: "asset_tag", value: "Asset Tag" },
  ...
  const handleArchive = async () => {
    try {
      await archiveAsset(asset).unwrap();
      enqueueSnackbar(
        `Asset has been ${params?.status === "active" ? "archived" : "restored"}`,
        { variant: "success" }
      );
      dispatch(resetModal());
  ...
  const handleImport = async () => {
    ...
    try {
      const res = await importAsset(mapped).unwrap();
      dispatch(resetModal());
  ...
  return (
    <Box sx={{ marginLeft: 2 }}>
      ...
      <AppPrompt
        open={archive}
        image={warning}
        title={`${params?.status === "active" ? "Archive" : "Restore"} asset?`}
        message={`Are you sure you want to ${params?.status === "active" ? "archive" : "restore"} this asset?`}
